...
if [ "$2" -ne 0 ]; then
	...
	echo "Creating $1/$FILE with $2 size"
	dd if=/dev/zero of="$1/$FILE" bs=1M count="$2"
	mkswap "$1/$FILE"
	chown root:root "$1/$FILE" # We don't want to create
	chmod 0600 "$1/$FILE" # A potential security risk
	touch "/system/jflte-gpe/swap_enabled"
	echo "$2" > "/system/jflte-gpe/swap_enabled" 
fi
